...
-- Paso 1: Eliminar vistas que dependen de numero_completo (solo si existen)
...
-- Paso 4: Recrear vista v_facturas_resumen (SIEMPRE existe)
...
-- Paso 5: Recrear vista v_envios_resumen (solo si tabla envios existe)
DO $$
BEGIN
...
-- Paso 6: Recrear vista v_reporte_consumos_vivienda (solo si existe)
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'envios') THEN
    EXECUTE '
...
-- Paso 7: Recrear vista v_facturas_pendientes_remesa (solo si tabla remesas_detalles existe)
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'remesas_detalles') THEN
    EXECUTE '
...
-- Paso 8: Recrear vista v_facturas_pendientes_envio (solo si tabla envios existe)
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'envios') THEN
    EXECUTE '
