-- =====================================================================
-- SCRIPT: BORRADO TOTAL DE 6 COMUNIDADES CON ERRORES DE IMPORTACION
-- =====================================================================
-- Comunidades afectadas:
--   Codigo 600 - 102 NAVALCARNERO
--   Codigo 537 - 36 VIV MARICARA
--   Codigo 544 - 57 VIV Q LIVING ARGANGA
--   Codigo 343 - 74 VIV BUTARQUE II
--   Codigo 351 - 102 VIV ERRIF TORREJON
--   Codigo 594 - 480 VIV GETAFE I
--
...
--   5. clientes           (solo exclusivos de estas comunidades)
...
FROM comunidades c
WHERE c.codigo IN ('600', '537', '544', '343', '351')
ORDER BY c.codigo;
...
WHERE factura_id IN (
  ...
  WHERE f.comunidad_id IN (
    ...
    WHERE codigo IN ('600', '537', '544', '343', '351', '594')
  )
);
...
-- ---------------------------------------------------------------------
-- PASO 5: Clientes exclusivos de estas comunidades
...
WHERE id IN (
  -- Clientes vinculados a estas comunidades
...
-- ---------------------------------------------------------------------
-- PASO 6: Comunidades (borrado raiz)
...
BEGIN
  ...
  IF v_count = 0 THEN
    RAISE NOTICE 'OK - BORRADO COMPLETADO: Las 6 comunidades han sido eliminadas correctamente.';
  ELSE
    RAISE EXCEPTION 'ERROR: Aun quedan % comunidad(es) sin borrar. Revisar manualmente y hacer ROLLBACK.', v_count;
  END IF;
END $$;
